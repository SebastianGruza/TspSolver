...
for ox in (1, 2):
    b, ok, dt = solve_ga(D, T=256, grid_epochs=150, use_merge=1, use_tabu=1, use_uniq=1,
                         seed=1, ls_mode=2, opt_start=0.6, ox_mode=ox, ox_topk=10, tag="ox%d" % ox)
    print("kroA100 ox_mode=%d top10: best=%d gap=%.3f%% perm=%s [%.0fs]" % (ox, b, (b/21282-1)*100, ok, dt), flush=True)
